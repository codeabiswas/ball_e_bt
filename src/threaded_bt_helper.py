...
class ThreadedBtHelper(QThread):
    """
    ...
    """
    bt_button_click = pyqtSignal(bool)
    ...
    def __init__(self, device_name="AB Shutter3"):
        ...
        # Keypress logging: It should toggle between True and False, the first input being True, and then the next is False
        self.keypress_type = False
        ...
        try:
            ...
            for event in dev.read_loop():
                ...
                if event.type == evdev.ecodes.EV_KEY:
                    ...
                    self.keypress_type = not self.keypress_type
                    self.bt_button_click.emit(self.keypress_type)
        except:
            logging.error("Could read BT input", exc_info=True)

    def disconnect_from_button(self):
        """Disconnects from button
        """
